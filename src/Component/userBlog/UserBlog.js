...
useEffect(() => {
    const fetchUserBlogs = async() => {
      try {
        const response = await axios.get(`http://localhost:8000/bloggingApplication/api/v1/blog/user/${user._id}`);
        ...
        >
          ...
          <div className="p-4 flex flex-col justify-between flex-1">
            <div>
              ...
              <p className="text-sm text-gray-500">{blog.createdAt?.slice(0, 10)}</p>
            </div>
